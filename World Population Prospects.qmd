...
# Visualizing Global Demographics: Insights from UN Population Prospects 2022

Authors: Zehra Cebeci, Omer Faruk Yahsi

Mail Address : [zehra.cebeci\@metu.edu.tr](zehra.cebeci@metu.edu.tr), [yahsi.omer\@metu.edu.tr](yahsi.omer@metu.edu.tr)
...
This term project provides an extensive examination of demographic variables through the use of data visualization and handling techniques. We aim to analyze and provide findings from a demographic dataset obtained from the United Nations Population Prospects \[1\]. The dataset encompasses a wide range of demographic information, such as population, life expectancy, mortality rates, and more.
...
Upon initial analysis, the results of the **`total_na_count`** indicated the absence of 7399 missing values. However, during the exploration of the scatter plot, a warning surfaced, indicating the presence of rows with missing values about 31885. This discrepancy prompted a closer examination of the data quality.
...
This particular time frame aligns with the infamous China's Great Leap Famine \[4\], during which approximately 55 million people succumbed to hunger over the course of these three years.
...
In addition, after obtaining information about the general attitudes towards birth rates, mortality, and population estimates in our nation, we have decided to examine the potential future  action regarding the complicated issue of refugee and migration issues, which is currently a significant concern in our country.

```{r}
selected_countries <- c("Türkiye")
filtered_data <- country_area_level_filtered |> 
  filter(country_and_area %in% selected_countries)

hc <- filtered_data |> 
  hchart(
  'line', hcaes(x = year, y = net_number_of_migrants_thousands, group = country_and_area)
  ) %>%
  hc_title(
    text = "<b>Total Number of Migrants</b>",
    margin = 20,
    align = "left",
    style = list(color = "#22A884", useHTML = TRUE)
  )%>%
  hc_xAxis(title = list(text = "Year")) %>%
  hc_yAxis(title = list(text = "Total Migrants (in Thousands)")) |> hc_xAxis(plotLines = list(list(value = 2022)))
...
```{r}
selected_countries <- c("Türkiye",  "United States of America", "Russian Federation", "United Kingdom", "France","China")
...
\[1\] <https://population.un.org/wpp/Download/Standard/MostUsed/>

\[2\] <https://www.iwm.org.uk/history/timeline-of-20th-and-21st-century-wars>

\[3\] <https://www.britannica.com/event/Persian-Gulf-War>

\[4\] <https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1127087/>

\[5\] [Rippe JM. Lifestyle Strategies for Risk Factor Reduction, Prevention, and Treatment of Cardiovascular Disease. Am J Lifestyle Med. 2018 Dec 2;13(2):204-212. doi: 10.1177/1559827618812395. PMID: 30800027; PMCID: PMC6378495.]{.underline}
