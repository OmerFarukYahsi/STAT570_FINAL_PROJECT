...
# Entries containing the term "region" (case-insensitive) are filtered, indicating the Sub-Region level.
# A new column 'sub_region' is created, capturing information from 'region_subregion_country_or_area.'
# Unnecessary columns are deselected, and 'sub_region' is relocated after the 'variant' column.
...
# Entries containing the terms "Region" or "World" (case-insensitive) are filtered, representing the Region level.
# A new column 'region' is created, extracting information from 'region_subregion_country_or_area.'
...
# Entries containing the term "Development Group" (case-insensitive) are filtered, indicating the Development Group level.
# A new column 'development_group' is created, capturing information from 'region_subregion_country_or_area.'
# Unnecessary columns are deselected, and 'development_group' is relocated after the 'variant' column.
...
# Entries containing the term "Income Group" (case-insensitive) are filtered, indicating the Income Group level.
# A new column 'income_group' is created, extracting information from 'region_subregion_country_or_area.'
# Unnecessary columns are deselected, and 'income_group' is relocated after the 'variant' column.
```
...
custom_limits <- c(0, 700000)

# Create the ggplot world map
ggplot(world_population) +
  geom_sf(aes(fill = total_population_as_of_1_july_thousands), color = "white", size = 0.2) +
  scale_fill_viridis_c(name = paste("Total Population (thousands) -", your_year), 
...
custom_limits <- c(0, 750)
custom_breaks <- seq(0, 750, by = 100)


# Create the ggplot world map
ggplot(world_population) +
  geom_sf(aes(fill = total_population_in_millions), color = "white", size = 0.2) +
  scale_fill_gradientn(name = paste("Total Population (thousands) -", your_year), 
                       colors = rainbow(20),
                       limits = custom_limits,
                       oob = scales::squish,
